...
class DashboardController extends Controller
{
  public function index()
  {
    ...
    foreach ($events as $event) {
      ...
      $lokasi = $event->lokasi;
      $budget = $event->budget;
      $list_kategori = DB::select('SELECT * FROM events_has_divisi WHERE events_id = ?', [$id]);
      
      $string_kategori = "";
      if (count($list_kategori) > 0) {
        for ($i = 0; $i < count($list_kategori); $i++) {
          if ($i != count($list_kategori) - 1) {
            $nama_divisi = Kategori::find($list_kategori[$i]->divisi_id)->nama;
            $string_kategori .= $nama_divisi .= ", ";
          } else {
            $nama_divisi = Kategori::find($list_kategori[$i]->divisi_id)->nama;
            $string_kategori .= $nama_divisi;
          }
